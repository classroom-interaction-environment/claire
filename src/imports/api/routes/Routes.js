...
Routes.codeRegister = {
	path: (queryParams) =>
		queryParams ? `/register?qp=${queryParams}` : "/register",
...
Routes.verifyEmail = {
	path: (verificationToken = ":verificationToken") =>
...
Routes.enrollAccount = {
	path: (verificationToken = ":verificationToken") =>
		`/enroll-account/${verificationToken}`,
...
const loggedInTrigger = createLoggedinTrigger(
	() => resolveRedirect() || Routes.root.path(),
);
