		...
		autoform: onClient({
			...
			options: (() => {
				...
				return () => {
					...
					const index = mappedRoles.findIndex(
						(entry) => entry.value === highest,
					);
...
export const passwordSchemaClassic = ({
	...
	autoform: onClient({
		hint:
			hint &&
			i18n.reactive("login.passwordHint", {
				min,
				max,
...
export const confirmSchema = ({
	...
	custom() {
		...
		if (!password || password.value !== confirm) {
			return (
				Schema.provider.ErrorTypes.NOT_EQUAL_CONFIRM ||
				Schema.provider.ErrorTypes.VALUE_NOT_ALLOWED
			);
		}
		return true;
	},
...
export const agreementSchema = ({
	...
	termsOfService: {
		type: Boolean,
		label:
			termsOfServiceLabel || i18n.reactive("agreements.termsOfService.read"),
	},
	privacyPolicy: {
		type: Boolean,
