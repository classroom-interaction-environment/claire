...
function build(settings) {
	...
	if (exists(target.min) && target.min.rule) {
		...
		rules.push({
			...
			message: i18n.reactive("login.password.minimumChars", {
				min: min.toString(),
			}),
		});
	}
	...
	if (exists(target.max) && target.max.rule) {
		...
		rules.push({
			...
			message: i18n.reactive("login.password.maximumChars", {
				max: max.toString(),
			}),
		});
	}
	...
	if (target.blacklist && target.blacklist.length > 0) {
		const blacklistMessage = (value) =>
			i18n.get("login.password.notEasyToGuess", { password: value });
		const blacklistRule = blacklist({
			list: target.blacklist,
