...
describe(UserFactory.name, () => {
	...
	describe("creation", () => {
		...
		it("creates a user optionally with or without password", async () => {
			...
			const withoutPasswordUser = await UsersCollection.findOneAsync(
				withoutPasswordUserId,
			);
			expect(withoutPasswordUser.services.password).to.equal(undefined);

			const withPasswordUserId = await UserFactory.create(
				userDoc({ password: `${Random.id()}1` }),
			);
			const withPasswordUser =
				await UsersCollection.findOneAsync(withPasswordUserId);
