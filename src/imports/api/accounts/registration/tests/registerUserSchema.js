...
describe("registerUserSchema", () => {
	describe(firstNameSchema.name, () => {
		...
		it("throws on non-name input", () => {
			...
			expect(() => firstName.validate({ value })).to.throw(
	...
	describe(lastNameSchema.name, () => {
		...
		it("throws on non-name input", () => {
			...
			expect(() => lastName.validate({ value })).to.throw(
				"form.validation.regEx",
				value,
			);
		});
	...
	describe(emailSchema.name, () => {
		...
		it("throws on non-email input", () => {
			...
			for (const value of fails) {
				expect(() => email.validate({ value })).to.throw(
					"form.validation.EmailWithTLD",
					value,
				);
			}
		});
	...
	describe(agreementSchema.name, () => {
		...
		it("throws on input that fails minimal requirements", () => {
			...
			for (const value of values) {
				expect(() => agree.validate(agree.validate(value))).to.throw(
					/.*/,
					value,
					value,
				);
	...
	describe(passwordSchemaClassic.name, () => {
		...
		it("throws on input that fails minimal requirements", () => {
			[
				...
				undefined,
				null,
				Random.id(passwordDef.min - 1),
				Random.id(passwordDef.max + 1),
				"”#£ﬁ^˜·¯&()=)(/&%",
			].forEach((value) => {
	...
	describe(confirmSchema.name, () => {
		...
		it("throws if input does not match password", () => {
			...
			[
				...
				expect(() => confirm.validate({ password, confirm: value })).to.throw(
					/.*/,
					value,
					value,
				);
