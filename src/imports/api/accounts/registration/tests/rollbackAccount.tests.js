...
describe(rollbackAccount.name, () => {
  ...
  it('removes a user from the account system', async () => {
    ...
    const { userRemoved, adminRemoved, rolesRemoved } = await rollbackAccount(userId)
  ...
  it('removes all roles from the user', async () => {
    ...
    expect(await count(Meteor.roleAssignment, { 'user._id': userId })).to.equal(2)
    ...
    const { userRemoved, rolesRemoved, adminRemoved } = await rollbackAccount(userId)
    expect(userRemoved).to.equal(1)
    expect(adminRemoved).to.equal(0)
    expect(rolesRemoved).to.equal(2)

    expect(await count(Meteor.roleAssignment, { 'user._id': userId })).to.equal(0)
