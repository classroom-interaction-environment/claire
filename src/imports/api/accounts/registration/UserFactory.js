...
UserFactory.create = async ({
	...
	try {
		...
		throw new Meteor.Error("createUser.failed", "createUser.notCreated", {
			email,
		});
	}
	...
	const profileUpdated = await getUsersCollection().updateAsync(
		userId,
		profileDoc,
	);
	...
	if (!isInRole) {
		await rollbackAccount(userId);
		throw new Meteor.Error("createUser.failed", "createUser.rolesNotAdded", {
			email,
		});
	}
