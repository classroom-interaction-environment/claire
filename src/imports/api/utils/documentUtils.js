...
export const ownerShip = deprecate(
	(document, userId) => !!(userId && document && document.createdBy === userId),
);
...
export const createGetDoc = deprecate(
	({ name } = {}, { checkOwner = true } = {}) => {
		check(name, String);
		/**
		...
		async function getDoc(_id) {
			...
			if (
				checkOwner &&
				!(ownerShip(document, this.userId) || (await isAdmin(this.userId)))
			) {
 ...
 */
export const createUpdateDoc = deprecate(
	({ name } = {}, { checkOwner = true } = {}) => {
		check(name, String);
		...
		function updateDoc(_id, { $set, $unset, $addToSet, $push, $pull }) {
			...
			if (
				checkOwner &&
				!(ownerShip(document, this.userId) || isAdmin(this.userId))
			) {
...
export const createRemoveDoc = deprecate(
	(
		{ name, isFilesCollection } = {},
		{ checkOwner = true, multiple = false } = {},
	) => {
		check(name, String);
