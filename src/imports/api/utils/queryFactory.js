export const createInQuery =
	(field) =>
	(values = []) => ({ [field]: { $in: values } });
export const createExistsQuery =
	(field) =>
	(value = false) => ({ [field]: { $exists: value } });

export const $existsQuery =
	(...values) =>
	(fieldName) =>
		createExistsQuery(fieldName).call(...values);
export const $inQuery =
	(...values) =>
	(fieldName) =>
		createInQuery(fieldName).call(...values);
