...
export const Resolvers = {
	...
	resolveFieldValue(key, value, referenceSchema) {
		...
		const resolvedValue = schemaEntry.resolve
			? schemaEntry.resolve(value)
			: value;

		// DEFAULT - RETURN RESOLVED VALUE
		if (
			resolvedType === DisplayTypes.string ||
			resolvedType === "number" ||
			resolvedType === "icon" ||
			resolvedType === "user"
		) {
		...
		if (resolvedType === "object") {
			throw new Error(
				"Resolvers.resolveFieldValue: resolvedType object not yet implemented",
			);
		}
		...
		if (resolvedType === "array") {
			if (!Array.isArray(resolvedValue)) {
				console.error(
					new Error(
						`resolveFieldValue: expected resolvedValue to be an array on ${key}`,
					),
				);
