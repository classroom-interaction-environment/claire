...
export const checkPermissions = (options) => {
	...
	options.run = async function run(...args) {
		...
		if (!userId) {
			throw new PermissionDeniedError("errors.userNotExists", {
				userId,
				name: options.name,
			});
		}
		...
		if (admin && !(await isAdmin(userId))) {
			throw new PermissionDeniedError("errors.userNotAdmin", {
				userId,
				name: options.name,
			});
		}
		...
		if (curriculum && !(await userIsCurriculum(userId))) {
			throw new PermissionDeniedError("errors.userNotCurriculum", {
				userId,
				name: options.name,
			});
		}
		...
		if (roles) {
			const scope =
				group || (await Meteor.users.findOneAsync(userId)).institution;
		...
		this.checkDoc = (document, docId, userId) => {
			console.warn(
				`[${options.name}]: this.checkDoc is deprecated, import it as standalone method!`,
			);
		...
		this.checkOwner = (document) => {
			console.warn(
				`[${options.name}]: this.checkOwner is deprecated, import it as standalone method!`,
			);
