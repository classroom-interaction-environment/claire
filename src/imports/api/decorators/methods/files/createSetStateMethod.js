...
const capitalize = (s, pos = 0) =>
	(s && `${s[pos].toUpperCase()}${s.slice(1)}`) || "";
...
export const createSetStateMethod = (context, { type = "master" } = {}) => {
	if (!allowed.includes(type)) {
		throw new TypeError(
			`Expected type to be one of ['master','custom'], got '${type}'`,
		);
	}
	...
	if (type === "master") {
		createUpdate = (value) =>
			value === true ? { $set: { _master: true } } : { $unset: { _master: 1 } };
	}

	if (type === "custom") {
		createUpdate = (value) =>
			value === true ? { $set: { _custom: true } } : { $unset: { _custom: 1 } };
	}
