...
if (Meteor.settings.patch?.removeDeadReferences) {
	Meteor.startup(async () => {
		const {
			Unit,
		} = require("../../../../contexts/curriculum/curriculum/unit/Unit");
		const {
			Pocket,
		} = require("../../../../contexts/curriculum/curriculum/pocket/Pocket");
		const { getCollection } = require("../../../../api/utils/getCollection");
		const {
			unitMaterialIds,
		} = require("../../../../contexts/curriculum/curriculum/unit/unitMaterialIds");
		...
		const unitMasterDocs = await UnitCollection.find({
			_master: true,
		}).fetchAsync();
		for (const unitDoc of unitMasterDocs) {
			...
			const materialEntries = Object.entries(material).filter(
				([, linkedIds]) => linkedIds && linkedIds.length > 0,
			);
