...
Meteor.startup(async () => {
  if ('sessionStorage' in window) {
    const templates = JSON.parse(sessionStorage.getItem('TemplatesDebug') || '[]')
    internal.templates = new Set(templates)
    console.debug('TemplatesDebug loadeded from sessionStorage:', templates, '=>', internal.templates)
  }
})

TemplatesDebug.debug = (name, remove = false) => {
  const fullName = name.startsWith('Template.')
    ? name
    : `Template.${name}`
  const value = remove
  ...
  if ('sessionStorage' in window) {
    sessionStorage.setItem('TemplatesDebug', JSON.stringify(Array.from(internal.templates)))
  }
  return value
}
