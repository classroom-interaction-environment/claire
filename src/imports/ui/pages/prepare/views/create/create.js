...
Template.createClass.onCreated(function () {
	...
	this.autorun(() => {
		const schoolClasses = getLocalCollection(SchoolClass.name).find({
			createdBy: userId,
		});
	...
	this.autorun(() => {
		...
		LessonMaterial.load(unitDoc).then((result) => {
...
Template.createClass.helpers({
	loadComplete() {
		return (
			API.initComplete() &&
			wizardInitialized.get() &&
			Template.getState("schoolClassesComplete")
		);
	},
	hasClasses() {
	...
	schoolClasses() {
		return cursor(() =>
			getLocalCollection(SchoolClass.name).find({ createdBy: Meteor.userId() }),
		);
	},
	createUnitDoc() {
	...
	myClasses() {
		return getLocalCollection(SchoolClass.name).find({
	...
	unitsForPocket(pocketId) {
		...
		return getLocalCollection(Unit.name)
			.find(query, { sort: { index: 1 } })
			.fetch()
			.filter((unitDoc) => {
				...
				return (
					unitDoc.dimensions.filter(
						(dimension) => !disabledDimensions.includes(dimension),
					).length > 0
				);
			});
	},
...
function completeWizard(templateInstance, onComplete) {
	...
	createLesson({ classId, unitId: originalUnitId }, ({ lessonId, unitId }) => {
		if (onComplete) {
			setTimeout(
				() => onComplete({ lessonId, classId, originalUnitId, unitId }),
				500,
			);
		}
	});
