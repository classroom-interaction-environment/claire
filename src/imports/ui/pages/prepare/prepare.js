...
Template.prepare.onCreated(async function () {
	...
	const skipLessonIds = new Set(
		LessonCollection.find().map((lessonDoc) => lessonDoc._id),
	);
	const unitIds = new Set(
		getLocalCollection(Unit.name)
			.find()
			.map((unitDoc) => unitDoc._id),
	);
...
Template.prepare.helpers({
	loadComplete() {
		...
		return state.get(lessonComplete) && state.get(schoolClassComplete);
	},
	submenuData() {
