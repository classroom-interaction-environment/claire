...
Template.taskProgress.helpers({
	currentProgress() {
		...
		const stateDoc = TaskWorkingStateCollection.findOne({
			createdBy,
			lessonId,
			taskId,
		});
	...
	offline(userDoc) {
		return (
			!userDoc.presence ||
			!userDoc.presence.status ||
			userDoc.presence.status !== "online"
		);
	},
});
...
const getProgress = ({ stateDoc, items, taskId, createdBy }) => {
	...
	const combinedProgress = {
		progress: typeof stateDoc.progress === "number" ? stateDoc.progress : 0,
		page: typeof stateDoc.page === "number" ? stateDoc.page + 1 : 1,
