...
Template.inviteUsers.helpers({
	loadComplete() {
		...
		return (
			instance.state.get("codesComplete") &&
			instance.state.get("myClassesSubscriptionsReady")
		);
	},
	codes() {
		return cursor(() =>
			getCollection(CodeInvitation.name).find({}, { sort: { createdAt: -1 } }),
		);
	},
	toDate(time) {
	...
	expiredCodes(codes) {
		return codes
			?.fetch()
			.filter((doc) => !invitationPending(doc))
			.sort((a, b) => {
...
Template.inviteUsers.events({
	...
	"click .delete-invitation-button"(event, templateInstance) {
		...
		confirmDialog({ text: "codeInvitation.confirmDelete", codeRequired: true })
			.then((result) => {
				...
				templateInstance.state.set("deleting", target);
				Meteor.call(
					CodeInvitation.methods.remove.name,
					{ _id: target },
					(err, _res) => {
						templateInstance.state.set("deleting", null);
						if (err) {
	...
	"submit #createInvitationForm": async (event, templateInstance) => {
		...
		const createNewClass =
			role === UserUtils.roles.student && (!classId || classId === "new");
	...
	"click .create-qr-button": async (event, templateInstance) => {
		...
		const updateCode = beamerDoc.invitationCode === code ? null : code;
		try {
