...
Template.userProfile.onCreated(function () {
	...
	const ensureProfileImage = (user) => {
		if (user.profileImage && !hasProfileImageLoaded(user.profileImage)) {
			...
			const onComplete = () =>
				setTimeout(() => this.state.set("loadingProfileImage", false), 300);
			loadProfileImage({
				user,
				onError: (err) => {
					API.notify(err);
...
Template.userProfile.helpers({
	...
	profileSchema(fieldName) {
		if (!profileSchemas[fieldName]) {
			...
			profileSchemas[fieldName] = Schema.create(
				{ [fieldName]: schema },
				{ tracker: Tracker },
			);
		}
...
Template.userProfile.events({
	...
	"change .research-option-select"(_event, templateInstance) {
		...
		const participateVisible = Array.from(formData.values()).every(
			(val) => !!val,
		);
