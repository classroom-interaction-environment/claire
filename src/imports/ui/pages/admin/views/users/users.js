...
const createUserSchema = Schema.create(Admin.methods.createUser.schema, {
	tracker: Tracker,
});
const updateSchemas = new Map();
updateSchemas.set(
	"role",
	Schema.create(Admin.methods.updateRole.schema, { tracker: Tracker }),
);
...
Template.adminUsers.events({
	...
	"click .delete-user-button"(event, templateInstance) {
		...
		const textOptions = {
			name: `${targetUser.firstName} ${targetUser.lastName} (${targetUser.emails[0].address})`,
		};
