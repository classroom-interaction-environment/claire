...
const isStudent = (userId) =>
	userId && UserUtils.getHighestRole(userId) === UserUtils.roles.student;
const ViewStates = {
	basics: "basics",
	username: "username",
	password: "password",
...
Template.codeRegister.onCreated(function () {
	...
	this.autorun(() => {
		if (this.wizard.isCurrentState(ViewStates.password)) {
			...
			passwordConfig.blacklist({
				...
				message: (value) =>
					i18n.get("login.password.noPreviousCredentials", { password: value }),
			});
	...
	try {
		...
		Meteor.call(
			CodeInvitation.methods.verify.name,
			{ code: parsed.code },
			(err, validCodeDoc) => {
				if (err) {
				...
				if (finalParams.classId && isStudent(userId)) {
					...
					return confirmDialog({
						...
						.then((result) => {
							if (result) {
								Meteor.call(
									CodeInvitation.methods.addToClass.name,
									{ code: finalParams.code },
									(err) => {
										if (err) {
		...
		this.fail(
			new Meteor.Error(
				"codeInvitation.invalidLink",
				"codeInvitation.invalidLinkReason",
			),
		);
	}
});
...
Template.codeRegister.events({
	...
	"submit #usernameForm"(event, templateInstance) {
		...
		Meteor.call(Users.methods.userIsAvailable.name, { email }, (err, res) => {
			...
			if (!res) {
				...
				templateInstance.wizard.pushView(
					ViewStates.password,
					ViewStates.username,
				);
			}
	...
	"submit #acceptTermsForm"(event, templateInstance) {
		...
		const passwordCredentials = templateInstance.state.get(
			"passwordCredentials",
		);
		const locale = currentLanguage()?.code ?? undefined;
		const registerWithCodeDoc = Object.assign(
			{},
			basicCredentials,
			passwordCredentials,
			usernameDoc,
		...
		Meteor.call(
			Users.methods.registerWithCode.name,
			registerWithCodeDoc,
			(registerError, userId) => {
				...
				if (!userId) {
					return templateInstance.fail(
						new Meteor.Error("codeInvitation.failed"),
					);
				}
