...
const getRunningLessons = ({ sort, limit }) =>
	getLocalCollection(Lesson.name).find(
		{
...
const getUpcomingLessons = ({ sort, limit }) =>
	getLocalCollection(Lesson.name).find(
		{
...
Template.ongoingLessons.helpers({
	...
	hasMoreUpcoming() {
		return (
			Template.getState("limitUpcoming") < Template.getState("maxUpcoming")
		);
	},
	unit(unitId) {
