...
Template.present.onCreated(function () {
	...
	this.autorun(() => {
		...
		getLocalCollection(Lesson.name)
	...
	this.autorun(() => {
		...
		const resolvedReferences = beamerDoc.references.map((ref) => {
			...
			if (ref.itemId) {
				...
				ref.availableResponseProcessors =
					availableResponseProcessors?.length > 1
	...
	this.autorun(() => {
		...
		for (const ref of resolvedReferences) {
			...
			if (cachedRefs.has(ref.itemId)) {
				...
				if (
					cached.responseProcessor &&
					cached.responseProcessor === ref.responseProcessor
				) {
					continue;
				}
			}
...
Template.present.onRendered(function () {
	this.autorun(() => {
		...
		const colorState = bgRef ? backgroundColors[bgRef] : backgroundColors.light;
...
Template.present.helpers({
	loadComplete() {
		...
		if (!taskInitialized || !rpInitialized) {
			return false;
		}
		...
		return beamerComplete && lessonsComplete && materialComplete;
	...
	previewTemplate({ context }) {
		return (
			Template.getState(context) &&
			LessonMaterial.getPreviewTemplate({ name: context })
		);
	},
...
Template.present.events({
	...
	"click .select-rp-button": async (event, templateInstance) => {
		...
		const beamerDoc = Beamer.doc.get();
		const index = beamerDoc.references.findIndex(
