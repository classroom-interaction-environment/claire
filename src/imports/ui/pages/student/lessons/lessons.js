...
Template.lessons.onCreated(function () {
	...
	instance.autorun(() => {
		...
		const lessonIds = LessonCollection.find()
			.fetch()
			.map((doc) => doc._id);
...
Template.lessons.helpers({
	loadComplete() {
		...
		return (
			instance.state.get("schoolClassesReady") &&
	...
	nonCurrentClasses() {
		...
		return (
			classDoc && SchoolClassCollection.find({ _id: $nin([classDoc._id]) })
		);
	},
	loadErrors() {
...
function updateClassId(
	{ user = Meteor.user(), classId },
	callback = defaultUpdateClassIdError,
) {
			...
			(err, unitDocs = []) => {
				if (err) {
					...
					if (err.error === "errors.docNotFound") {
						...
						const corruptedLessonDocs = LessonCollection.find({
							unit: err.reason,
						})
							.fetch()
							.map((doc) => doc._id);
						...
						warn(
							"there were errors loading units for the following docs",
							corruptedLessonDocs,
						);
