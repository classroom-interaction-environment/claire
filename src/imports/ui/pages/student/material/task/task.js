...
Template.task.onCreated(function () {
	...
	instance.onLoadError = (loadError) => {
		console.error(
			instance.viewName || instance.view?.name,
			"load error",
			loadError,
		);
	...
	instance.autorun(() => {
		...
		if (taskDoc) {
			...
			const allVisible = (lessonDoc.visibleStudent || []).concat(
				groupDoc?.visible || [],
			);
			loadStudentMaterial({
				_id: lessonDoc._id,
			...
			callbacks: {
				...
				onReady: async () => {
					...
					if (LessonStates.isRunning(lessonDoc)) {
						...
						const page = 0;
						const progress = 0;
						const taskWorkingStateInsertDoc = {
							lessonId,
							taskId,
							complete,
							page,
	...
	instance.autorun(() => {
		...
		Meteor.call(
			Lesson.methods.units.name,
			{ lessonIds: [lessonDoc._id] },
			(loadError, unitDocs) => {
				if (loadError) {
...
Template.task.helpers({
	...
	taskData() {
		...
		const complete = lessonIsRunning
			...
			: false;
		const page = lessonIsRunning ? (taskWorkingDoc.page ?? 0) : 0;
		const onError = API.notify;
		const itemHandlers = {
			onItemLoad: ItemHandlers.onItemLoad({
				instance,
				TaskResultCollection,
				onError,
			}),
			onItemSubmit:
				lessonIsRunning && ItemHandlers.onItemSubmit({ instance, onError }),
			onTaskPageNext:
				lessonIsRunning && ItemHandlers.onTaskPageNext({ instance, onError }),
			onTaskPagePrev:
				lessonIsRunning && ItemHandlers.onTaskPagePrev({ instance, onError }),
			onTaskFinished:
				lessonIsRunning && ItemHandlers.onTaskFinished({ instance, onError }),
		};
		...
		return {
			...
			page: page,
			data: Object.assign(
				{ preview: false, print: false, student: true },
				taskDoc,
				itemHandlers,
			),
