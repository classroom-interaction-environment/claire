 ...
 */
LessonMaterial.subscribe = (unitDoc, callback) =>
	setTimeout(() => {
...
function subscribe(unitDoc, callback) {
	...
	const queue = (target, context) => {
		if (target && target.length > 0) {
			...
			console.error(
				"deprecated: use API.subscribe instead of direct SubsManager calls",
			);
			return SubsManager.subscribe(
				context.publications.editor.name,
				{ _id: $in(target) },
				{
					onError: callback,
				},
	...
	const allSubs = [];
	const allLoaded = () =>
		Object.values(_loaded).every((entry) => entry === true);
	...
	Object.entries(unitDoc).forEach(([fieldName, materialIds]) => {
		...
		if (!context) {
			callback(
				new Error(
					`Expected Material-context by name [${contextName}] for field [${fieldName}]`,
				),
			);
		} else {
			allSubs.push({
 ...
 */
LessonMaterial.getPreviewData = function getPreviewData({
	materialDoc,
	templateInstance,
	options,
}) {
	...
	const document =
		getLocalCollection(name).findOne(referenceId) ||
		getCollection(name).findOne(referenceId);
