...
Template.afUnitSelect.onCreated(function () {
	...
	this.autorun(() => {
		...
		const disabledDimensionsList = Object.entries(disabledDimensions)
		...
		const pocketDocs = getLocalCollection(Pocket.name).find(
			{ _master: true, _custom: null },
			{ sort: { title: 1 } },
		);
		...
		for (const pocketDoc of pocketDocs) {
			...
			const units = getLocalCollection(Unit.name)
				.find(selector)
				.fetch()
				.filter((unitDoc) => {
					const lessonDoc = getLocalCollection(Lesson.name).findOne({
						classId,
						unitOriginal: unitDoc._id,
					});
...
Template.afUnitSelect.helpers({
	...
	dimensions(ids) {
		return (
			ids &&
			cursor(() =>
				getLocalCollection(Dimension.name).find({ _id: { $in: ids } }),
			)
		);
	},
	allDimension() {
...
Template.afUnitSelect.events({
	"click .select-unit"(event, templateInstance) {
		...
		const selectedUnit = unitId === currentUnit ? "" : unitId;
