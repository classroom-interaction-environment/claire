...
Template.afUserGroupSelect.onCreated(function () {
	...
	const materialOptions = (allMaterial || []).filter(({ value }) =>
	...
	this.autorun(() => {
		...
		this.state.set({
			users: resolvedUsers,
			hasUsers: resolvedUsers.length > 0,
		});
	});
});
...
const getUsers = ({ users, assignedUsers }) => {
	return users
		...
		.sort(
			(a, b) =>
				(b.presence?.status === "online" ? 1 : 0) -
				(a.presence?.status === "online" ? 1 : 0),
		);
};
...
Template.afUserGroupSelect.events({
	...
	"click .material-toggle"(event, templateInstance) {
		...
		const index = Number.parseInt(
			dataTarget(event, templateInstance, "index"),
			10,
		);
	...
	"dragstart .user-element"(event, templateInstance) {
		...
		event.originalEvent.dataTransfer.setData(
			"application/groupBuilder-userId",
			userId,
		);
		event.originalEvent.dataTransfer.setData(
			"application/groupBuilder-groupIndex",
			groupIndex || "",
		);
	...
	"drop .user-dropzone"(event, templateInstance) {
		event.preventDefault();
		// the user id
		const userId = event.originalEvent.dataTransfer.getData(
			"application/groupBuilder-userId",
		);

		// index of the group the user came from
		// if -1 the user is "new" and came from no group
		const fromGroupIndexStr = event.originalEvent.dataTransfer.getData(
			"application/groupBuilder-groupIndex",
		);
		const fromGroupIndex = fromGroupIndexStr
			? Number.parseInt(fromGroupIndexStr, 10)
			: -1;
		...
		if (index === -1) {
			templateInstance.builder.removeUser({
				index: fromGroupIndex,
				userId,
				role,
			});
		}
