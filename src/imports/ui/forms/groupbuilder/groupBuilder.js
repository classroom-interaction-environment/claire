...
Template.groupBuilder.helpers({
	...
	groupsDoc() {
		const groups = Tracker.nonreactive(() =>
			Template.instance().builder.getAllGroups(),
		);
		return { groups };
	},
	editGroup(index) {
...
Template.groupBuilder.events({
	...
	"click .grp-init-btn"(event, templateInstance) {
		...
		groupsSchema = Schema.create(
			editGroupSchema(builder, templateInstance.data),
		);
