...
const editUnitSchema = API.createSchema(
	unitCreateSchema({
...
Template.curriculumPockets.helpers({
	...
	dimensions(arr = []) {
		return cursor(() =>
			getCollection(Dimension.name).find({ _id: { $in: arr } }),
		);
	},
	dragging() {
...
Template.curriculumPockets.events({
	...
	"click .add-unit-button,.edit-unit-button"(event, templateInstance) {
		...
		if (pocketId) {
			const index =
				getCollection(Unit.name).find({ pocket: pocketId }).count() + 1;
	...
	"click .delete-pocket-button"(event, templateInstance) {
		...
		const unitCount = getCollection(Unit.name)
			.find({ _master, pocket: pocketId })
			.count();
