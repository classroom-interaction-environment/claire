...
Template.taskPageContent.onRendered(function onTemplateRendered() {
	...
	instance.autorun(() => {
		...
		if (
			typeof selectedTargetIndex !== "undefined" &&
...
Template.taskPageContent.helpers({
	...
	attributesSchema(contentType, metaType) {
		...
		const schemaDef = TaskDefinitions.helpers.schema(contentType, metaType, {
			taskId,
		});
	...
	attributesDoc(preview, target) {
		...
		if (header) {
			return Object.assign({}, contentObject, {
				preview: true,
				isEditable: true,
			});
		}
	...
	hideAddContent() {
		const instance = Template.instance();
		return (
			instance.state.get("dragging") || instance.state.get("editAttributes")
		);
	},
	isSelected(targetId) {
	...
	newElement(content, meta) {
		...
		return (
			newElementState &&
			newElementState.content === content &&
			newElementState.meta === meta
		);
	},
	newElementFormState() {
...
Template.taskPageContent.events({
	...
	"click .add-content-entry"(event, templateInstance) {
		...
		let index = typeof rawIndex !== "undefined" ? parseInt(rawIndex, 10) : -1;
	...
	"submit #newElementForm"(event, templateInstance) {
		...
		if (
			newContent.type === Files.name &&
			Files.has(newContent.meta) &&
			isCurriculumDoc(task) &&
			userIsCurriculum()
		) {
	...
	"submit #attributesForm"(event, templateInstance) {
		...
		const currentContent = templateInstance.state.get("currentContent");
		const contentIndex = currentContent.findIndex(
			({ id }) => id === editTarget,
		);
		...
		if (editPage) {
			...
			updateDoc = {
				[`pages.${pageIndex}.content.${contentIndex}`]:
					task.pages[pageIndex].content[contentIndex],
			};
		}
	...
	"click .page-content-delete"(event, templateInstance) {
		...
		confirmDialog({ text })
			.then((result) => {
				...
				if (
					contentElement.type === Files.name &&
					Files.has(contentElement.meta)
				) {
					const filesDoc = getLocalCollection(contentElement.meta).findOne(
						contentElement._id,
					);
					...
					if (linkedTask === task._original) {
						...
						removeContextDoc({
							...
							prepare: () =>
								templateInstance.state.set("submitting", contentIndex),
							failure: (er) => API.notify(er),
							success: () => API.notify("file.deleted"),
						});
					} else {
						API.log(
							"skip removing doc as it has an unknown task linked:",
							linkedTask,
						);
					}
				}
	...
	"click .page-content-move"(event, templateInstance) {
		...
		if (header) {
			...
			updateDoc = {
				header: task.header,
				[`pages.${targetPageIndex}.content`]:
					task.pages[targetPageIndex].content,
			};
		} else if (footer) {
			...
			updateDoc = {
				...
				[`pages.${currentPageIndex}.content`]:
					task.pages[currentPageIndex].content,
			};
		}
