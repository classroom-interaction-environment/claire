 ...
 * @return {*|null}
 */
export const createSelectableMaterialEntriesQuery = (
	viewState,
	unitDoc,
	unitDocOriginal,
	...
	let originalUnit =
		unitDocOriginal ||
		getCollection(Unit.name).findOne(unitDoc._original) ||
		getLocalCollection(Unit.name).findOne(unitDoc._original);

	if (!originalUnit) {
		// send a warning only in case the unit is not a master doc
		if (!unitDoc._master) {
			console.warn(
				`Found no original unit [${unitDoc._original}] for current unit [${unitDoc._id}]`,
			);
		}
