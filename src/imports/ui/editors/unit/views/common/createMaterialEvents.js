...
export const createMaterialEvents = ({ API, onMaterialCreated = noop }) => {
	...
	return {
		...
		"click .uematerial-remove-button"(event, templateInstance) {
			...
			const title =
				materialDoc.title || materialDoc.name || i18n.get(context.label);
				...
				.then((result) => {
					...
					updateContextDoc({
						...
						success: () => {
								...
								removeReferences(
									...
									(err, _phaseDoc) => {
										if (err) {
											...
											return API.notify(
												"editor.unit.material.unlinkedFromPhase",
											);
										}
									},
								);
							...
							if (deleteMaterial) {
								Meteor.call(
									context.methods.remove.name,
									{ _id: targetId },
									(err) => {
										if (err) {
											API.notify(err);
										} else {
											getLocalCollection(context.name).remove({
												_id: targetId,
											});
											API.notify(
												i18n.get("editor.unit.material.deleted", { title }),
											);
										}
									},
								);
				...
				onCreated: (materialId) => {
					setTimeout(() => {
						...
						onMaterialCreated({
							materialId,
							templateInstance,
							isMasterMaterial: !!insertDoc._master,
						});
					}, 500);
				},
			});
