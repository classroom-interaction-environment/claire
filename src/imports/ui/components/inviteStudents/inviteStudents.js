...
const hidden = (obj) =>
	Object.assign({}, obj, { autoform: { type: "hidden" } });
...
Template.inviteStudents.onCreated(function () {
	...
	this.getEncodedEmail = (invitationDoc, link) => {
		...
		const to =
			invitationDoc?.firstName && invitationDoc.lastName
		...
		API.subscribe({
			...
			callbacks: {
				...
				onReady: () => {
					...
					const codeDoc = CodeInvitationCollection.findOne(
						{ classId },
						{ sort: { createdAt: -1 } },
					);
					...
					const invitationExpiredOrComplete =
						codeDoc &&
						(invitationExpired(codeDoc) || invitationComplete(codeDoc));
					const registeredUsers = (codeDoc.registeredUsers || []).map(
						(userId) => {
...
Template.inviteStudents.helpers({
	activeInvitation() {
		...
		const { classId } = data;
		if (!classId) return;
		return CodeInvitationCollection.findOne(
			{ classId },
			{ sort: { createdAt: -1 } },
		);
	},
	...
	loadComplete() {
		const instance = Template.instance();
		return (
			instance.state.get("loadComplete") && instance.state.get("usersLoaded")
		);
	},
	showList() {
...
Template.inviteStudents.events({
	...
	"click .refresh-invitation-button"(event, templateInstance) {
		...
		Meteor.call(
			CodeInvitation.methods.forceExpire.name,
			{ _id },
			delayedCallback(300, (err) => {
