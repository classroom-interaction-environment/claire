...
Template.formModal.helpers({
	...
	formIsBlocked() {
		...
		return (
			["remove", "view"].includes(formState.action) ||
			Template.getState("submitting")
		);
	},
	hideLegend() {
		const formState = state.get() || {};
...
export const FormModal = {
	show: (options) => {
		...
		if (typeof custom === "object") {
			formState.custom = Object.entries(custom).map(([key, value]) => ({
				key,
				...value,
			}));
		...
		formState.schema =
			typeof schema === "function" ? schema({ ...bind }) : schema;
