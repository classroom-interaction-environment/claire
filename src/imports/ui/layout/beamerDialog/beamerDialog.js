...
Template.beamerDialog.helpers({
	...
	beamerBackground() {
		...
		return (
			background && ` h-100 bg-${background.className} text-${background.text}`
		);
	},
	presentLoaded() {},
	initVisible() {
	...
	controlVisible() {
		return (
			Template.getState("presentLoaded") && Template.getState("controlVisible")
		);
	},
});
...
Template.beamerDialog.events({
	...
	"click .open-beamer-external-button"(event, templateInstance) {
		event.preventDefault();
		if (!templateInstance.state.get("presentLoaded")) {
			import("../../pages/present/present").then(() =>
				templateInstance.state.set("presentLoaded", true),
			);
		}
		templateInstance.$("#beamerInitDialog").modal("hide");
