...
describe(Unit.name, () => {
	...
	let UsersCollection;
	before(() => {
		[UnitCollection, , UsersCollection] = mockCollections(
			Unit,
			Phase,
			Users,
			Admin,
	...
	describe("methods", () => {
		describe(Unit.methods.byTaskId.name, () => {
			...
			it("returns all units that reference a task by id if curriculum user", async () => {
				...
				const result1 = await byTaskId.call(
					{ userId },
					{
				...
				const unitDoc2 = await createUnit({
					tasks: [Random.id(), Random.id()],
				});
				const result2 = await byTaskId.call(
					{ userId },
					{
			...
			it("returns otherwise only those units that a user owns", async () => {
				...
				const unitDoc = await createUnit({
					createdBy: userId,
					tasks: [Random.id(), Random.id()],
				});
		...
		describe(Unit.methods.unlinkTask.name, () => {
			...
			it("unlinks a task from a unit doc if owner", async () => {
				...
				const modifiedCount = await unlink.call(
					{ userId },
					{ taskId: taskIdToUnlink },
				);
		...
		describe(Unit.methods.remove.name, () => {
			...
			it("removes the unitDoc", async () => {
				...
				const materialIds = unitMaterialIds(
					await UnitCollection.findOneAsync(unitId),
				);
				...
				const { unitRemoved, materialRemoved } = await removeUnit.call(
					{ userId },
					{ _id: unitId },
				);
