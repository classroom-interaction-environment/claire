...
const optionalUserType = () => {
	...
	return maxUsers && maxUsers === 1 ? "text" : "hidden";
};
...
CodeInvitation.schema = {
	...
	institution: {
		...
		autoform: {
			...
			type: () => {
				const userId = Meteor.userId();
				return isAdmin(userId) ? "text" : "hidden";
			},
		},
	},
	classId: {
		...
		autoform: {
			...
			options() {
				...
				const allCourses = getSchoolClass()
					.find({ createdBy: userId }, { sort: { title: 1 } })
					.map((doc) => ({
						value: doc._id,
						label: doc.title,
					}));
...
CodeInvitation.methods.remove = {
	...
	schema: { _id: String },
	run: onServer(({ _id }) =>
		getCollection(CodeInvitation.name).removeAsync(_id),
	),
};
