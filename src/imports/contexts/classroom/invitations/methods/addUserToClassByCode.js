...
export const addUserToClassByCode = async ({ code, userId }) => {
  ...
  try {
    ...
    const err = new PermissionDeniedError(CodeInvitation.errors.invalidCode, { code, userId })
    err.cause = e
    throw err
  }
  if (!validateInvitation(codeDoc)) {
    throw new PermissionDeniedError(CodeInvitation.errors.invalidCode, { code, userId })
  }
  ...
  if (!await hasRole(userId, role, user.institution)) {
    throw new PermissionDeniedError(PermissionDeniedError.notInRole, { role, userId })
  }
  ...
  if (role === Hierarchy.teacher) {
    ...
    await UsersCollection.updateAsync(userId, { $set: { 'ui.classId': classId } })
  }
  else {
    throw new PermissionDeniedError(SchoolClass.errors.invalidRole, { role, userId })
  }
