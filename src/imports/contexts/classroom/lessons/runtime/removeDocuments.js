...
const removeTaskResults = createDocRemover({
	name: TaskResults.name,
	...options,
});
const removeTaskWorkingState = createDocRemover({
	name: TaskWorkingState.name,
	...options,
});
...
export const removeDocuments = async ({
	...
	// tasks
	removed[TaskResults.name] = await removeTaskResults({
		query: docQuery,
		userId,
		debug,
	});
	removed[TaskWorkingState.name] = await removeTaskWorkingState({
		query: docQuery,
		userId,
		debug,
	});
	...
	// files
	removed[ImageFiles.name] = await removeImageFiles({
		query: fileQuery,
		userId,
		debug,
	});
	removed[AudioFiles.name] = await removeAudioFiles({
		query: fileQuery,
		userId,
		debug,
	});
	removed[DocumentFiles.name] = await removeDocumentFiles({
		query: fileQuery,
		userId,
		debug,
	});
	removed[VideoFiles.name] = await removeVideoFiles({
		query: fileQuery,
		userId,
		debug,
	});
