...
export const toggleLessonMaterial = async ({
	lessonId,
	userId,
	referenceId,
	context,
	...
	if (!LessonStates.canToggle(lessonDoc)) {
		throw new Meteor.Error(
			...
			"lesson.expectedToggleAble",
			{
				lessonId,
				startedAt: lessonDoc.startedAt,
				completedAt: lessonDoc.completedAt,
			},
		);
	}
	...
	const index = (lessonDoc.visibleStudent || []).findIndex(
	...
	// in case of unexpected state we throw
		...
		throw new Meteor.Error(
			"common.error",
			LessonErrors.unexpectedMaterialIndex,
			{ index },
		);
	}
