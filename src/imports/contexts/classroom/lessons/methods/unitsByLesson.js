...
export const unitsByLesson = async ({ lessonIds, userId }) => {
	...
	const lessonDocs = await getCollection(Lesson.name)
	...
	const classDocs = await getCollection(SchoolClass.name)
	...
	return getCollection(Unit.name)
		.find({ _id: $in(unitsIds) })
		.fetchAsync();
};
