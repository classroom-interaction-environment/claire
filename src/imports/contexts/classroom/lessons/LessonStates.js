...
const isCompleted = ({ startedAt, completedAt }) =>
	!!(startedAt && completedAt);

const canEdit = ({ startedAt, completedAt }) =>
	isIdle({ startedAt, completedAt });

const canStart = ({ startedAt, completedAt }) =>
	isIdle({ startedAt, completedAt });

const canStop = ({ startedAt, completedAt }) =>
	isRunning({ startedAt, completedAt });

const canComplete = ({ startedAt, completedAt }) =>
	isRunning({ startedAt, completedAt });

const canResume = ({ startedAt, completedAt }) =>
	isCompleted({ startedAt, completedAt });

const canRestart = ({ startedAt, completedAt }) =>
	isRunning({ startedAt, completedAt }) ||
	isCompleted({ startedAt, completedAt });

const canToggle = ({ startedAt, completedAt }) =>
	canRestart({ startedAt, completedAt });
