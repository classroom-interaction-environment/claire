...
describe("lesson runtime helpers", () => {
	...
	describe(resetBeamer.name, () => {
		...
		it("returns 0 if there are no references on the beamer doc", async () => {
			const query = { lessonId: Random.id(), userId: Random.id() };
			await BeamerCollection.insertAsync({
		...
		it("returns the diff if there are references related to the lessonId", async () => {
			...
			const { lessonRefs, nonLessonRefs } = randomReferences(
				insertDoc,
				query.lessonId,
			);
	...
	describe(resetGroups.name, () => {
		...
		it("removes all ad-hoc groups", async () => {
			...
			const removeGroupId = await GroupCollection.insertAsync(
				createGroupDoc({ unitId, title: "to remove", isAdhoc: true }),
			);
			const otherGroupId = GroupCollection.insert(
				createGroupDoc({ unitId, title: "other", isAdhoc: false }),
			);
			...
			expect(
				await GroupCollection.countDocuments({ _id: removeGroupId }),
			).to.equal(0);
			expect(
				await GroupCollection.countDocuments({ _id: otherGroupId }),
			).to.equal(1);
		...
		it("resets groups that are defined on a unit-level", async () => {
			const lessonId = Random.id();
			const unitId = Random.id();
			const updateGroupId = await GroupCollection.insertAsync(
				createGroupDoc({
					title: "to update",
					unitId,
			...
			const otherGroupId = await GroupCollection.insertAsync(
				createGroupDoc({
					title: "other",
					unitId: Random.id(),
					isAdhoc: false,
			...
			expect(await GroupCollection.countDocuments({})).to.equal(2);
			expect(await GroupCollection.findOneAsync(otherGroupId)).to.deep.equal(
				otherDoc,
			);
		});
	});
