	...
	describe("methods", () => {
		...
		describe(Lesson.methods.create.name, () => {
			...
			it("creates a new lesson doc", async () => {
				...
				const { lessonId, unitId } = await createLesson.call(
					{ userId, log },
					{ classId, unitId: originalUnit },
				);
			...
			it("creates a copy of the given master unit", async () => {
				...
				const classId = Random.id();
				const lessonCreateDoc = {
					classId,
					unitId: unitOriginal._id,
					createdBy: userId,
				};
			...
			it("creates copies of the given master phases", async () => {
				...
				const { lessonId } = await createLesson.call(
					{ userId, log },
					lessonCreateDoc,
				);
				const lessonDoc = await LessonCollection.findOneAsync(lessonId);
		...
		describe(Lesson.methods.toggle.name, () => {
			...
			it("throws if the material is not existent", async () => {
				...
				const { lessonDoc, userId } = await stubTeacherDocs({
					lessonProps: { startedAt: new Date(), unit },
				});
				await expectThrow({
					fn: () =>
						toggleMaterial.call(
			...
			it("pushes material to the list, if not visible", async () => {
				const { lessonDoc, userId } = await stubTeacherDocs({
					lessonProps: { startedAt: new Date() },
				});
		...
		describe(Lesson.methods.restart.name, () => {
			...
			it("removes all visible references", async () => {
				...
				lessonDoc.visibleStudent = [
					{ _id: Random.id(), context: Random.id(5) },
				];
			...
			it("resets all groups, associated with this lesson", async () => {
				...
				await getCollection(Group.name).insertAsync(
					createGroupDoc({ title: "not remove", unitId }),
				);
				await getCollection(Group.name).insertAsync(
					createGroupDoc({ title: "to remove", unitId, isAdhoc: true }),
				);

				const { groupDocs } = await restartLesson.call(
					{ userId, log },
					lessonDoc,
				);
		...
		describe(Lesson.methods.remove.name, () => {
			...
			it("removes lesson", async () => {
				...
				await LessonCollection.insertAsync(lessonDoc);
				expect(await count(LessonCollection, { _id: lessonDoc._id })).to.equal(
					1,
				);
			...
			it("does not remove master unit", async () => {
				...
				const { lessonRemoved, unitRemoved } = await removeLesson.call(
					{ userId, log },
					{ _id: lessonDoc._id },
				);
			...
			it("removes cloned phases", async () => {
				...
				const unitDoc = await mockUnitDoc(
					{ phases: [phaseDoc._id], createdBy: userId },
					UnitCollection,
				);
				const unitId = unitDoc._id;
				...
				const { phasesRemoved } = await removeLesson.call(
					{ userId, log },
					{ _id: lessonDoc._id },
				);
			...
			it("does not remove global phases or master phases", async () => {
				...
				const othersPhaseId = (
					await mockPhaseDoc({ unit: unitId }, PhaseCollection)
				)._id;
				const globalPhaseId = (
					await mockPhaseDoc({ createdBy: userId }, PhaseCollection)
				)._id;
				const masterPhaseId = (
					await mockPhaseDoc({ _master: true }, PhaseCollection)
				)._id;
				expect(othersPhaseId).to.be.a("string");
				expect(globalPhaseId).to.be.a("string");
				expect(masterPhaseId).to.be.a("string");
				await UnitCollection.updateAsync(unitId, {
					$set: {
						phases: [phaseId, othersPhaseId, globalPhaseId, masterPhaseId],
					},
				});
				unitDoc = await UnitCollection.findOneAsync(unitId);
				expect(unitDoc.phases).to.deep.equal([
					phaseId,
					othersPhaseId,
					globalPhaseId,
					masterPhaseId,
				...
				// before
				expect(await count(PhaseCollection, { _id: phaseId })).to.equal(1);
				expect(await count(PhaseCollection, { _id: othersPhaseId })).to.equal(
					1,
				);
				expect(await count(PhaseCollection, { _id: globalPhaseId })).to.equal(
					1,
				);
				expect(await count(PhaseCollection, { _id: masterPhaseId })).to.equal(
					1,
				);
			...
			it("removes cloned material", async () => {
				...
				const taskId = await TaskCollection.insertAsync({
					createdBy: userId,
					title: Random.id(),
				});
				...
				const { materialRemoved } = await removeLesson.call(
					{ userId, log },
					{ _id: lessonDoc._id },
				);
			...
			it("removes groups, associated with this lesson", async () => {
				...
				await getCollection(Group.name).insertAsync(
					createGroupDoc({ title: "to remove", unitId }),
				);
				await getCollection(Group.name).insertAsync(
					createGroupDoc({ title: "to remove", unitId, isAdhoc: true }),
				);

				const { groupsRemoved } = await removeLesson.call(
					{ userId, log },
					{ _id: lessonDoc._id },
				);
		...
		describe(Lesson.methods.material.name, () => {
			...
			it("returns undefined if no material is considered visible", async () => {
				...
				expect(
					await getLessonMaterial.call({ userId, log }, lessonDoc),
				).to.equal(undefined);
			});
			...
			it("returns the material, referenced by a lesson doc", async () => {
				...
				const materialDocs = await getLessonMaterial.call(
					{ userId, log },
					lessonDoc,
				);
			...
			it("indicates if there are docs not found, but referenced in the material", async () => {
				...
				expect(materialDocs).to.deep.equal({
					[Task.name]: [],
					notFound: [{ context: Task.name, _id: taskId }],
				});
			});
		...
		describe(Lesson.methods.units.name, () => {
			...
			it("returns all units by given lesson ids", async () => {
				...
				const lessonDocs = lessonIds.map((lessonId, index) => ({
					_id: lessonId,
					classId,
					unit: unitIds[index],
				}));
				...
				const foundUnitDocs = await getUnits.call(
					{ userId, log },
					{ lessonIds },
				);
