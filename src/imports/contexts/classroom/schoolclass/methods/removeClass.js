...
export const removeClass = async ({ classId, userId, log = noop }) => {
	...
	const associatedLessons = await LessonCollection.find({
		classId,
		createdBy: userId,
	}).fetchAsync();
	...
	if (
		(await LessonCollection.countDocuments({ classId, createdBy: userId })) > 0
	) {
		throw new Meteor.Error(
			"schoolClass.errors.removeFailed",
			"schoolClass.errors.failedToRemoveLessons",
		);
	}
