    ...
    describe('methods', () => {
      ...
      describe(SchoolClass.methods.create.name, () => {
        it('creates a new school class doc', async () => {
          ...
          const classDocId = await createClass.call(environment, Object.assign({}, classDocDef))
      ...
      describe(SchoolClass.methods.remove.name, () => {
        it('throws if the classDoc is not found', async () => {
          await expectThrow({
            fn: () => removeClass.call({ userId: Random.id() }, { _id: Random.id() }),
            error: DocNotFoundError.name
          })
        })
        it('throws if the user is not allowed to remove the class', async () => {
          const userId = Random.id()
          const classDoc = { title: Random.id(), createdBy: Random.id(), teachers: [userId], students: [] }
        ...
        it('removes the class (and only this class) by given _id', async () => {
          ...
          const classId = await SchoolClassCollection.insertAsync(classDoc)
          const otherClassId = await SchoolClassCollection.insertAsync(classDoc)

          await removeClass.call({ userId }, { _id: classId })
          expect(await count(SchoolClassCollection, { _id: classId })).to.equal(0)
          expect(await count(SchoolClassCollection, { _id: otherClassId })).to.equal(1)
        })
        it('removes all lessons of this (and only this) class', async () => {
          ...
          lessons.fill(0)
          lessons = await Promise.all(lessons.map(() => LessonCollection.insertAsync({
            classId,
            title: Random.id(),
            createdBy: userId,
            unit: Random.id()
          ...
          otherLessons.fill(0)
          otherLessons = await Promise.all(otherLessons.map(() => LessonCollection.insertAsync({
          ...
          for (const lessonId of lessons) {
            expect(await count(LessonCollection, { _id: lessonId })).to.equal(1)
          }
          for (const lessonId of otherLessons) {
            expect(await count(LessonCollection, { _id: lessonId })).to.equal(1)
          }
          expect(await count(LessonCollection, { classId })).to.equal(lessons.length)
          ...
          // after
          for (const lessonId of lessons) {
            expect(await count(LessonCollection, { _id: lessonId })).to.equal(0)
          }
          for (const lessonId of otherLessons) {
            expect(await count(LessonCollection, { _id: lessonId })).to.equal(1)
          }
      ...
      describe(SchoolClass.methods.addStudent.name, () => {
        ...
        it('throws if the user to be added is already member of the class', async () => {
          ...
          const classId = await SchoolClassCollection.insertAsync(classDoc)
          await expectThrow({
            fn: () => addStudent.call({ userId }, { classId, userId: studentId }),
            error: PermissionDeniedError.name,
        ...
        it('adds the user as student to the class', async () => {
          ...
          const updatedClass = await SchoolClassCollection.findOneAsync(classId)
      ...
      describe(SchoolClass.methods.removeStudent.name, () => {
        ...
        it('throws if the user to be added is NOT member of the class', async () => {
          ...
          const classId = await SchoolClassCollection.insertAsync(classDoc)
          await expectThrow({
            fn: () => removeStudent.call({ userId }, { classId, userId: studentId }),
            error: PermissionDeniedError.name,
