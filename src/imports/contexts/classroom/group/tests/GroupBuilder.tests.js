...
describe("GroupBuilder", () => {
	describe("constructor", () => {
		it("can be instatiated with optional defaults", () => {
			expect(new GroupBuilder().groupTitleDefault).to.equal(
				"group.defaultTitle",
			);
			expect(
				new GroupBuilder({ groupTitleDefault: "foo" }).groupTitleDefault,
			).to.equal("foo");
	...
	describe(GroupBuilder.prototype.addUser.name, () => {
		...
		onInvalidGroupIndex((builder) =>
			builder.addUser({ index: 2, userId: "foo" }),
		);
	...
	describe(GroupBuilder.prototype.updateUser.name, () => {
		onInvalidUserId((builder) => builder.addUser({ index: 2, userId: "moo" }));
		onInvalidGroupIndex((builder) =>
			builder.updateUser({ index: 2, userId: "foo" }),
		);
		...
		it("updates the given user", () => {
			...
			expect(builder.getGroup(0).users).to.deep.equal([
				{ userId, role: "oink" },
			]);
		});
	});
	describe(GroupBuilder.prototype.removeUser.name, () => {
		onInvalidUserId((builder) => builder.addUser({ index: 2, userId: "moo" }));
		onInvalidGroupIndex((builder) =>
			builder.removeUser({ index: 2, userId: "foo" }),
		);
		it("throws if the user is NOT within that group", () => {
			onInvalidUser({
				fn: (builder) => {
