...
class GroupBuilder {
	...
	setOptions(options) {
		...
		this.materialForAllGroups =
			options.materialForAllGroups ?? this.materialForAllGroups;
		this.materialAutoShuffle =
			options.materialAutoShuffle ?? this.materialAutoShuffle;
		this.atLeastOneUserRequired =
			options.atLeastOneUserRequired ?? this.atLeastOneUserRequired;
	...
	createGroups({ shuffle = false }) {
		...
		if (this.atLeastOneUserRequired && usersCount === 0) {
			throw new Meteor.Error(
				"groupBuilder.error",
				"groupBuilder.atLeastOneUserRequired",
			);
		}
			...
			if (this.materialAutoShuffle && material.length > 0) {
				...
				// and distribute multiple material per group
					...
					for (let j = from; j < to; j++) {
						const index = j > material.length - 1 ? j - material.length : j;
	...
	addMaterial({ index, materialId }) {
		...
		if (group.material.includes(materialId)) {
			throw new Meteor.Error(
				"groupBuilder.error",
				"groupBuilder.expectedNoMaterial",
			);
		}
	...
	addUser({ index, userId, role }) {
		...
		const userIndex = groups[index].users.findIndex(
			(doc) => doc.userId === userId,
		);
		if (userIndex > -1) {
			throw new Meteor.Error(
				"groupBuilder.error",
				"groupBuilder.expectedNoUser",
			);
		}
...
const checkGroupOptions = (options) =>
	check(
		options,
