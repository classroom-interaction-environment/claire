...
Users.methods.resendVerificationMail = {
	...
	run: onServerExec(() => {
		...
		} = require("./methods/resendVerificationEmail");
...
Users.methods.sendResetPasswordEmail = {
	...
	run: onServerExec(() => {
		...
		} = require("./methods/sendResetPasswordEmail");
...
Users.methods.registerWithCode = {
	...
	schema: auto(() => {
		const {
			PasswordConfig,
		} = require("../../../../api/accounts/registration/PasswordConfig");
...
Users.methods.byClass = {
	...
	roles: [
		...
		UserUtils.roles.schoolAdmin,
		UserUtils.roles.teacher,
		UserUtils.roles.student,
	],
	schema: {
		classId: String,
		skip: {
