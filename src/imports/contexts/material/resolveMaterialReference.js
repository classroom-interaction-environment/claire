 ...
 * @return {null|object}
 */
export const resolveMaterialReference = (
	refObj,
	{ preferLocal = false } = {},
) => {
	...
	const resolvedDocument = preferLocal
		? (LocalCollection.findOne(document) ??
			MaterialCollection.findOne(document))
		: (MaterialCollection.findOne(document) ??
			LocalCollection.findOne(document));

	if (!resolvedDocument) {
		console.warn(
			"[resolveMaterialReference]: could not resolve document for ",
			collection,
			document,
		);
		return null;
