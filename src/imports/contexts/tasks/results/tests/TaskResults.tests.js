		...
		describe(TaskResults.methods.saveTask.name, () => {
			...
			const stubDocuments = async ({
				...
				stub(
					TaskCollection,
					"findOneAsync",
					async () => hasTask && { _id: createDoc.taskId },
				);
				stub(
					SchoolClassCollection,
					"findOneAsync",
			...
			it("throws if the lesson does not exists", async () => {
				const { createDoc, lessonId, userId } = await stubDocuments({
					hasLesson: false,
				});
				await expectThrow({
			...
			it("updates the response document if one exists already for the given item", async () => {
				...
				const docId = await TaskResultCollection.insertAsync({
					createdBy: userId,
					...createDoc,
				});
				expect(await count(TaskResultCollection)).to.equal(1);
				const updateDoc = Object.assign({}, createDoc, {
					response: [Random.id()],
				});
