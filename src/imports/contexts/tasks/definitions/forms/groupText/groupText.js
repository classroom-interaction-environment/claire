...
Template.afGroupText.onCreated(function () {
	...
	instance.getMemberUpdates = ({ reactive = false } = {}) =>
		reactive
	...
	instance.autorun(() => {
		...
		instance.observer = getCollection(TaskResults.name)
			.find({ itemId, createdBy: { $ne: userId } })
			.observeChanges({
				added(id, doc) {
					...
					if (!user) {
						return console.warn(
							`Observer: added taskresult doc but found no user for doc ${id}`,
						);
					}
				...
				changed(id) {
					const memberUpdates = instance.getMemberUpdates({ reactive: false });
					const [foundUserId] = Object.entries(memberUpdates).find(
						([_id, entry]) => entry.docId === id,
					);

					if (!foundUserId) {
						return console.warn(
							`Observer: updated taskresult doc but found no user for doc ${id}`,
						);
					}
...
Template.afGroupText.onRendered(function () {
	this.autorun((c) => {
		...
		const myDoc = getCollection(TaskResults.name).findOne({
			itemId,
...
Template.afGroupText.helpers({
	...
	memberResponse(userId) {
		...
		const responseDoc = getCollection(TaskResults.name).findOne({
			itemId,
			createdBy: userId,
		});
...
Template.afGroupText.events({
	...
	"click .group-member-tab"(event, templateInstance) {
		...
		const memberUpdates = templateInstance.getMemberUpdates({
			reactive: false,
		});
