...
ResponseProcessorLoader.initialize = ({
 ...
 */
ResponseProcessorLoader.importTemplate = async function importTemplate({
	name,
	renderer,
}) {
 ...
 * @return {Object|*}
 */
ResponseProcessorLoader.get = function get({
	dataType,
	responseProcessor,
	meta,
	fileType,
 ...
 */
ResponseProcessorLoader.loadAll = async ({
	item,
	lessonId,
	taskId,
	itemId,
	...
	const template =
		await ResponseProcessorLoader.importTemplate(responseProcessor);
	const data = Object.assign({}, item);
	data.lessonDoc = getCollection(Lesson.name).findOne(lessonId);
	data.results = getCollection(TaskResults.name)
		.find({
			itemId,
			taskId,
			lessonId,
