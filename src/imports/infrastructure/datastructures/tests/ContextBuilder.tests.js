...
describe("ContextBuilder", () => {
	...
	describe(ContextBuilder.addContext.name, () => {
		it("throws if object is not a valid context", () => {
			expect(() => ContextBuilder.addContext({})).to.throw(
	...
	describe(ContextBuilder.addRegistry.name, () => {
		it("throws if not a valid registry", () => {
			expect(() => ContextBuilder.addRegistry({})).to.throw(
				"Match error: Missing key 'name'",
			);
			expect(() => ContextBuilder.addRegistry({ name: Random.id() })).to.throw(
				"Match error: Missing key 'hasIdentity'",
			);
			expect(() =>
				ContextBuilder.addRegistry({
					name: Random.id(),
					hasIdentity: () => {},
				}),
