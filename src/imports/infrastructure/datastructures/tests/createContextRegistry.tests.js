...
describe(createContextRegistry.name, () => {
	it("throws if options is not correct", () => {
		[
			...
			expect(() => createContextRegistry(options)).to.throw(
	...
	it("provides basic registry functionality", () => {
		...
		// expected errors
		expect(() => reg.add({})).to.throw("Match error: Missing key 'name'");
		expect(() => reg.add(ctx)).to.throw(
			`[${regName}]: expected [${ctxName}] to have identity for this registry`,
		);
