 ...
 */
export const buildPipeline = createPipeline(
	"build",
	(context, api, options) => {
		check(
			options,
		...
		const { collection, filesCollection, methods, publications, debug } =
			options;
		const products = {
			collection: null,
		...
		if (
			collection &&
			!context.collection &&
			isSupportedObject(context.schema)
		) {
		...
		if (filesCollection && isFilesContext(context)) {
			const FilesMongoCollection =
				products.collection ?? new Mongo.Collection(context.name);
			const { files } = context;
		...
		if (publications && isSupportedObject(context.publications)) {
			products.publications = Object.values(context.publications).map(
				(pubDef) => {
