...
export class ValidatedMethod {
  ...
  _execute(methodInvocation = {}, args) {
    ...
    methodInvocation.name = self.name;

    const validation = this.validate.bind(methodInvocation)(args);
    const handle = (validateResult) => {
      if (typeof validateResult !== 'undefined') {
        throw new Error(`Returning from validate doesn't do anything; \
perhaps you meant to throw an error?`);
      }

      return self.run.bind(methodInvocation)(args);
    }

    // Support both synchronous and asynchronous validation functions
    if (typeof validation?.then === 'function') {
      return validation.then(handle);
    } else {
